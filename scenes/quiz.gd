...
func _ready():
	if GameManager.is_hard_mode:
		set_mode(true)  # Active le mode Difficile
	else:
		set_mode(false)  # Mode Normal
...
func _on_answer_selected(index):
	...
	var time_taken = question_start_time - timer.time_left  # Temps utilisé pour répondre
	if time_taken < 0:
		time_taken = 0  # Empêcher un bug de temps négatif
	
	# Calcul des points
	var points = 1000 - int(time_taken * 100)  # On enlève 100 points par seconde
	if points < 100:
		points = 100  # Minimum 100 points si réponse correcte

	# Vérifier si la réponse est correcte
	if selected_answer == questions[current_question]["correct"]:
		total_score += points  # Ajouter les points seulement si la réponse est bonne
		correct_answers += 1  # Incrémenter le compteur de bonnes réponses
